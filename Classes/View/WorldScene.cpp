...
bool WorldScene::init(std::string levelName, int currentLevel, SoundController* soundController) {
	...
	for (auto spirit : world_->getSpirits()) {
		//spirit->getTexture()->setScale(0.5);
	...
	if (!isPause_)
	{
		...
		//this->schedule(schedule_selector(WorldScene::updateIsgameover), 15.0f);
	}
	
	over = Label::createWithTTF("Game over", "fonts/Marker Felt.ttf", 25);
	over->setPosition(Vec2(200,200));
	over->setColor(Color3B::RED);
...
//void WorldScene::updateIsgameover(float dt)
//{
//	if (worldController_->getIsGameOver())
//	{
//		worldLayer_->addChild(over, 2);
//		//worldController_->setBoolgameover(false);
//	}
//}
