...
void WorldController::updateWorld(float dt) {
	if (!isPause) {
		...
		if (world->isVictory()) {
			...
			UserDefault::getInstance()->setIntegerForKey(LevelMenuScene::parseLevel(world->getCurrentLevel()).c_str(), record);// ham chuyen diem sang levelmenu de lưu 
